...
fdfor:	sll $t0, $t0, 2			#i=i*4;
	bge $t0, $a2, fdendf		#for(i=0;i<size){
	...
	li  $t1, 0			#j=0;
	li  $t2, 1			#token=1;
fdfor2:	sll $t1, $t1, 2			# j=j*4;
	bge $t1, $a2, fdendf2		#for(j=0,token=1; j<size){
	
	add $t6, $t1, $a0		#array+j;
	lw  $t8, 0($t6)			#array[j];
	
	bne $t7, $t8, fdendiff		#if(array[i]==array[j]){
	sw  $t2, 0($t3)			# dup_array[j]=token;
	addi $t2, $t2, 1		# token++;
					
fdendiff:addi $t1, $t1, 1		#j++;
	j  fdfor2				#}

fdendf2:addi $t0, $t0, 1		#i++;
	j  fdfor				#}	
